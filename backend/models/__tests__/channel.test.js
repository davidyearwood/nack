...
test("It should add new channel", () => {
  ...
  expect(channel.addChannel("test-channel", "tester").name).toBe(
    "test-channel"
  );
...
test("It should add a new message", () => {
  const channel = new Channel(store);
  const message = {
    channelId: 1,
    sender: "test",
  ...
  const newMessage = channel.addMessage(message);
  expect(newMessage.sender).toEqual("test");
  expect(newMessage.msg).toEqual("testing testing");
  expect(channel.all()[0].msgs).toHaveLength(2);
});
